...
static int ReverbHICreateDpl2(AXFX_REVHI_WORK_DPL2* rv, f32 coloration, f32 time, f32 mix,
                              f32 damping, f32 preDelay)
{
    u8 i;
    u8 k;

    static s32 lens[9] = { 0x000006FD, 0x000007CF, 0x0000091D, 0x000001B1, 0x00000095,
                           0x0000002F, 0x00000049, 0x00000043, 0x00000047 };

    ASSERTMSGLINE(117,
                  coloration >= 0.0f && coloration <= 1.0f && time >= 0.01f && time <= 10.0f &&
                      mix >= 0.0f && mix <= 1.0f && damping >= 0.0f && damping <= 1.0f &&
    ...
    if ((coloration < 0.0f) || (coloration > 1.0f) || (time < 0.01f) || (time > 10.0f) ||
        (mix < 0.0f) || (mix > 1.0f) || (damping < 0.0f) || (damping > 1.0f) || (preDelay < 0.0f) ||
        (preDelay > 0.1f))
    {
        return 0;
    }
...
static int ReverbHIModifyDpl2(AXFX_REVHI_WORK_DPL2* rv, f32 coloration, f32 time, f32 mix,
                              f32 damping, f32 preDelay)
{
    u8 i;
...
asm static void HandleReverbDpl2(register s32* sptr, register AXFX_REVHI_WORK_DPL2* rv,
                                 register s32 k)
{
    nofralloc;
    stwu r1, -0xc0(r1);
...
int AXFXReverbHiInitDpl2(AXFX_REVERBHI_DPL2* reverb)
{
    ...
    ret = ReverbHICreateDpl2(&reverb->rv, reverb->coloration, reverb->time, reverb->mix,
                             reverb->damping, reverb->preDelay);
...
int AXFXReverbHiSettingsDpl2(AXFX_REVERBHI_DPL2* rev)
{
    ...
    old = OSDisableInterrupts();
    rev->tempDisableFX = 1;
    ret = ReverbHIModifyDpl2(&rev->rv, rev->coloration, rev->time, rev->mix, rev->damping,
                             rev->preDelay);
    rev->tempDisableFX = 0;
    OSRestoreInterrupts(old);
    return ret;
...
void AXFXReverbHiCallbackDpl2(AXFX_BUFFERUPDATE_DPL2* bufferUpdate, AXFX_REVERBHI_DPL2* reverb)
{
    ASSERTMSGLINE(
        1399, AXGetMode() == 2,
        "AX mode isn't AX_MODE_DPL2. AX mode must be AX_MODE_DPL2 for using AXFXReverbHiCallbackDpl2");
