...
static int ReverbSTDCreate(AXFX_REVSTD_WORK* rv, f32 coloration, f32 time, f32 mix, f32 damping,
                           f32 predelay);
static int ReverbSTDModify(AXFX_REVSTD_WORK* rv, f32 coloration, f32 time, f32 mix, f32 damping,
                           f32 predelay);
...
int AXFXReverbStdInit(AXFX_REVERBSTD* rev)
{
    ...
    rev->tempDisableFX = 0;
    ret = ReverbSTDCreate(&rev->rv, rev->coloration, rev->time, rev->mix, rev->damping,
                          rev->preDelay);
...
int AXFXReverbStdSettings(AXFX_REVERBSTD* rev)
{
    ...
    old = OSDisableInterrupts();
    rev->tempDisableFX = 1;
    ret = ReverbSTDModify(&rev->rv, rev->coloration, rev->time, rev->mix, rev->damping,
                          rev->preDelay);
    rev->tempDisableFX = 0;
    OSRestoreInterrupts(old);
    return ret;
...
void AXFXReverbStdCallback(AXFX_BUFFERUPDATE* bufferUpdate, AXFX_REVERBSTD* reverb)
{
    ...
    {
        ReverbSTDCallback(bufferUpdate->left, bufferUpdate->right, bufferUpdate->surround,
                          &reverb->rv);
    }
}
