...
static void CheckVertexPacket(void)
{
    ...
    if (__GXVertexPacketHas(GX_VA_CLR1) && !__GXVertexPacketHas(GX_VA_CLR0) &&
        __gxVerif->verifyLevel >= __gxvWarnLev[70])
    {
    ...
    if ((BYTE3(__gxVerif->xfRegs[8]) & 3) == 0 && ((BYTE3(__gxVerif->xfRegs[8]) >> 2) & 3) == 0 &&
        (u32)((BYTE3(__gxVerif->xfRegs[8]) >> 4) & 0xF) == 0)
    {
        numMatrixIndices = 0;
    ...
    for (i = 0; i < numRegularTextures; i++)
    {
        ...
        case 1:
            if (!__GXVertexPacketHas(GX_VA_NRM) && !__GXVertexPacketHas(GX_VA_NBT) &&
                __gxVerif->verifyLevel >= __gxvWarnLev[73])
            {
        ...
        case 12:
            if (!__GXVertexPacketHas(
                ...
                __gxVerif->verifyLevel >= __gxvWarnLev[77])
            {
    ...
    while (done == 0)
    {
        ...
        else if ((u32)((BYTE3(__gxVerif->xfRegs[count + 64]) >> 4) & 7) != 1)
        {
            if (!((BYTE3(__gxVerif->xfRegs[count + 64]) >> 4) & 7) &&
                __gxVerif->verifyLevel >= __gxvWarnLev[79])
            {
...
static void CheckBumpmapTextures(void)
{
    ...
    if (!__GXVertexPacketHas(GX_VA_PNMTXIDX))
    {
        if ((u32)(BYTE3(__gxVerif->xfRegs[24]) & 0x3F) > 30 &&
            __gxVerif->verifyLevel >= __gxvWarnLev[0x50])
        {
    ...
    for (i = 0; i < numBumpmapTextures; i++)
    {
        ...
        if ((BYTE3(__gxVerif->xfRegs[BumpMapSource + 64]) >> 4) & 7 &&
            __gxVerif->verifyLevel >= __gxvWarnLev[0x51])
        {
        ...
        if (!__gxVerif->xfLightDirty[lightRAMOffset - 0x600 + 0] &&
            __gxVerif->verifyLevel >= __gxvWarnLev[0x52])
        {
    ...
    for (i = 0; i < numRegularTextures; i++)
    {
        ...
        case 12:
            if ((BYTE3(__gxVerif->xfRegs[i + 64]) >> 2) & 1 &&
                __gxVerif->verifyLevel >= __gxvWarnLev[0x79])
            {
            ...
            (u32)((BYTE3(__gxVerif->xfRegs[16]) >> 1) & 1) == 1)
        {
            ...
            if (haveLight != 0)
            {
                if (!((BYTE2(__gxVerif->xfRegs[14]) >> 2) & 1) &&
                    ...
                    __gxVerif->verifyLevel >= __gxvWarnLev[0x59])
                {
                     ...
                     ((u32)((BYTE2(__gxVerif->xfRegs[16]) >> 2) & 1) == 1)))
                {
                    if ((__GXVertexPacketHas(GX_VA_NRM) == 0) &&
                        ...
                        __gxVerif->verifyLevel >= __gxvWarnLev[0x5A])
                    {
                        __GX_WARNF(0x5A, 0);
                    }
...
static void CheckDMatrixRAMRanges(void)
{
    ...
    for (i = 1280; i <= 1535; i++)
    {
        ...
        CheckFloatingPointValue(__gxVerif->xfDMtxDirty[i - 1280], __gxVerif->xfDMtx[i - 1280],
                                label);
    }
}
...
static void CheckLightRAMRanges(void)
{
    ...
    for (lightSource = 0; lightSource < 8; lightSource++)
    {
        for (i = 1; i < 13; i++)
        {
            ...
            sprintf(label, "Light %d %s (address 0x%04x)", lightSource, lightRegisterNames[i],
                    lightRAMOffset);
...
static void CheckErrors(void)
{
    ...
    {
        if (!(u32)(BYTE3(__gxVerif->xfRegs[9]) & 3) && !__gxVerif->xfRegs[0x3F] &&
            __gxVerif->verifyLevel >= __gxvWarnLev[0x38])
        {
            __GX_WARN(0x38);
        }
        ...
        if (numColorTextures != 0 &&
            (u32)((BYTE3(__gxVerif->xfRegs[numRegularTextures + numBumpmapTextures + 64]) >> 4) &
                  7) != 2 &&
            __gxVerif->verifyLevel >= __gxvWarnLev[0x68])
        {
            __GX_WARN(0x68U);
        }
