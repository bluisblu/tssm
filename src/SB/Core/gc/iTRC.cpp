...
BOOL ROMFont::Init() {
  if (OSGetFontEncode() == 1) {
    ROMFont::mFontData =
        (OSFontHeader *)OSAllocFromHeap(__OSCurrHeap, 0xF00 + 0x12);
  } else {
    ROMFont::mFontData =
        (OSFontHeader *)OSAllocFromHeap(__OSCurrHeap, 0x2 + 0x120);
  }
...
void ROMFont::InitGX() {
  U16 scaledHeight;
  GXSetViewport(0.0f, 0.0f, mRenderMode->fbWidth, mRenderMode->efbHeight, 0.0f,
                1.0f);
  GXSetScissor(0, 0, mRenderMode->fbWidth, mRenderMode->efbHeight);

  scaledHeight = GXSetDispCopyYScale(
  ...
  GXSetCopyFilter(mRenderMode->aa, mRenderMode->sample_pattern, TRUE,
                  mRenderMode->vfilter);
...
void ROMFont::RenderBegin() {
  if (mRenderMode->field_rendering != FALSE) {
    GXSetViewportJitter(0.0f, 0.0f, mRenderMode->fbWidth,
                        mRenderMode->efbHeight, 0.0f, 1.0f, VIGetNextField());
  } else {
    GXSetViewport(0.0f, 0.0f, mRenderMode->fbWidth, mRenderMode->efbHeight,
                  0.0f, 1.0f);
  }
...
// FIXME: Revisit after GXTexCoord2s16 and GXPosition3s16 have been decompiled,
// looks like they were inlined
...
void ROMFont::LoadSheet(void *image_ptr) {
  ...
  GXInitTexObj(&GStack_68, image_ptr, mFontData->sheetWidth,
               mFontData->sheetHeight, (GXTexFmt)mFontData->sheetFormat,
               GX_CLAMP, GX_CLAMP, NULL);
  GXInitTexObjLOD(&GStack_68, GX_LINEAR, GX_LINEAR, 0.0f, 0.0f, 0.0f, FALSE,
                  FALSE, max_aniso);
...
S32 ROMFont::DrawString(S32 param_1, S32 param_2, char *string) {
  ...
  while (*string != '\0') {
    string =
        OSGetFontTexture(string, image, (s32 *)&x, (s32 *)&y, (s32 *)&width);
...
void ROMFont::DrawTextBox(S32 param_1, S32 param_2, S32 param_3, S32 param_4,
                          char *str) {
...
void ResetButton::EnableReset() { ResetButton::mResetEnabled = 1; }

void ResetButton::DisableReset() { ResetButton::mResetEnabled = 0; }
...
bool iTRCDisk::Init(void) { return ROMFont::Init(); }

void iTRCDisk::SetErrorMessage(const char *message) {
  strcpy(mMessage, message);
}

void iTRCDisk::ResetMessage() { memset(mMessage, 0, 0x100); }

void iTRCDisk::SetPadStopRumblingFunction(void (*Func)()) {
  mPadStopRumbling = Func;
}

void iTRCDisk::SetSndSuspendFunction(void (*Func)()) { mSndSuspend = Func; }

void iTRCDisk::SetSndResumeFunction(void (*Func)()) { mSndResume = Func; }

void iTRCDisk::SetSndKillFunction(void (*Func)()) { mSndKill = Func; }
...
bool iTRCDisk::IsDiskIDed() { return false; }

void iTRCDisk::DisplayErrorMessage() {}

void iTRCDisk::SetDVDState() {}

bool iTRCDisk::CheckDVDAndResetState() { return false; }
